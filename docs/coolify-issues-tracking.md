# Coolify Upstream Issues Tracked by Corelix Platform
...
- **Relevance:** This is the foundational request. Corelix Platform implements exactly this — project-level and environment-level RBAC with View Only, Deploy, and Full Access tiers.

**Draft Comment:**

> Built this as a drop-in addon: [corelix-platform](https://github.com/corelixio/platform).
>
...
- **Status:** Open, on hold
- **Relevance:** Directly requests per-project member assignment. Corelix Platform's `project_user` pivot table with role-based overrides covers this exactly.

**Draft Comment:**

> This is implemented in [corelix-platform](https://github.com/corelixio/platform) — project-level member assignment with View Only / Deploy / Full Access permissions, plus optional environment-level overrides.
>
...
- **Status:** Open
- **Relevance:** This is a direct consequence of Coolify's policies returning `true` for all operations. Corelix Platform replaces all resource policies (Project, Application, Service, Database, Environment, Server) with permission-checked versions.
...
>
> [corelix-platform](https://github.com/corelixio/platform) replaces these with permission-checked policies. `ProjectPolicy::delete()` requires Full Access on the project. The policies are registered in `$app->booted()` to ensure they override Coolify's defaults (package providers boot before app providers).
>
...
- **Status:** Pull request
- **Relevance:** Corelix Platform independently implements per-storage S3 path prefix, plus adds transparent encryption on top. The rclone-based approach handles both prefix and encryption in a single pipeline.

**Draft Comment:**

> Path prefix is implemented in [corelix-platform](https://github.com/corelixio/platform) alongside transparent S3 encryption.
>
...
> If you're using [corelix-platform](https://github.com/corelixio/platform) with filename encryption enabled, note that retention cleanup routes through rclone instead of Laravel's S3 driver — rclone decrypts the filenames transparently before applying retention logic. This avoids the mismatch where the S3 driver sees encrypted filenames and fails pattern matching.
>
...
- **Status:** Open
- **Relevance:** Corelix Platform adds a `ResourceBackupManager` Livewire component that appears on every resource's Configuration page (Application, Service, Database) plus a server-level backup page. Supports volume, configuration, full, and Coolify instance backup types.

**Draft Comment:**

> Implemented in [corelix-platform](https://github.com/corelixio/platform) as the Resource Backup Manager.
>
...
>
> [corelix-platform](https://github.com/corelixio/platform) takes a different approach for resource-level restores — each resource's configuration is exported as structured JSON (model attributes, environment variables, persistent storages, docker-compose, custom labels). The Restore page lets you inspect the JSON and selectively import environment variables into existing resources, rather than overwriting the entire database.
>
...
>
> [corelix-platform](https://github.com/corelixio/platform) exports resource configuration as structured JSON — including git source references, environment variables, and deployment settings. The restore workflow lets you review and selectively import what makes sense for the new environment. Git source setup still requires manual GitHub App re-installation, but the export preserves the repository URLs and branch configurations so you know exactly what to reconnect.
...
>
> [corelix-platform](https://github.com/corelixio/platform) overlays the Import.php Livewire component to add encryption-aware restore, but this is scoped to standalone databases. For ServiceDatabase import, the addon's Resource Backup approach (volume-level tar snapshots) is the workaround — restore by replacing the data volume rather than running SQL import.
...
>
> [corelix-platform](https://github.com/corelixio/platform) overlays `DatabaseBackupJob` with improved error handling — meaningful exceptions instead of silent returns, and better status tracking. For databases large enough to cause timeout issues, the addon also offers volume-level backups (Resource Backups feature) as an alternative — `tar czf` on the data volume is typically faster than a logical dump for 50GB+ databases.
...
>
> For context — [corelix-platform](https://github.com/corelixio/platform) adds custom template sources (external GitHub repos as template origins) and follows the same write-once pattern. The template is consumed at deploy time and has zero runtime dependency. Removing a template source doesn't affect deployed services. Post-deployment editing is entirely a function of Coolify's service UI, which reads from `docker_compose_raw`.
...
> Broken built-in templates are stuck until the next Coolify release ships a fix. This is one of the reasons [corelix-platform](https://github.com/corelixio/platform) supports custom template sources — you can point to a GitHub repo with a corrected Authentik template and deploy it today, without waiting for the upstream fix.
>
...
>
> [corelix-platform](https://github.com/corelixio/platform) lets you add a GitHub repo with corrected templates as a custom source. The corrected PostHog template shows up in the New Resource page immediately. No Coolify source changes needed. See the [custom template guide](https://github.com/corelixio/platform/blob/main/docs/custom-templates.md) for the YAML format.
...
> If you need Jitsi working now — [corelix-platform](https://github.com/corelixio/platform) supports custom template sources from GitHub repos. Fork or create a repo with a fixed Jitsi template (same YAML format as Coolify's built-in), add it as a source in Settings > Templates, and deploy from there. The template format is documented at https://github.com/corelixio/platform/blob/main/docs/custom-templates.md.
...
> SurrealDB can be deployed today using [corelix-platform](https://github.com/corelixio/platform) in two ways:
>
> 1. **Custom template source** — Create a GitHub repo with a SurrealDB docker-compose template (with or without TiKV), add it as a source in Settings > Templates. Template format is identical to Coolify's built-in YAML. See the [template guide](https://github.com/corelixio/platform/blob/main/docs/custom-templates.md).
>
...
> Rather than waiting for a built-in template — [corelix-platform](https://github.com/corelixio/platform) supports custom template sources from GitHub repos. Create a repo with a WordPress + OpenLiteSpeed docker-compose template, add it as a source in Settings > Templates, and it appears in the New Resource page.
>
> The template format is the same YAML used by Coolify's built-in templates. Documentation and example: https://github.com/corelixio/platform/blob/main/docs/custom-templates.md
...
>
> [corelix-platform](https://github.com/corelixio/platform) supports custom template sources where you can host a MongoDB replica set template in a GitHub repo. The `coolify.database: "true"` label ensures each `mongod` container is classified correctly as a `ServiceDatabase`. Template format docs: https://github.com/corelixio/platform/blob/main/docs/custom-templates.md
...
> [corelix-platform](https://github.com/corelixio/platform) partially addresses this:
>
...
>
> [corelix-platform](https://github.com/corelixio/platform) fixes this three ways:
...
>
> [corelix-platform](https://github.com/corelixio/platform) lets you force classification with the `coolify.database` Docker label:
...
>
> Workaround with [corelix-platform](https://github.com/corelixio/platform): add `coolify.database: "false"` as a label on the InfluxDB service in your compose YAML. This forces `ServiceApplication` classification, which supports FQDNs. The label is checked by `isDatabaseImageEnhanced()` before the image name matching.
...
>
> [corelix-platform](https://github.com/corelixio/platform) overlays `StartDatabaseProxy` with:
...
>
> [corelix-platform](https://github.com/corelixio/platform) overlays `StartDatabaseProxy` for expanded port mapping. The timeout value could be made configurable via an environment variable or per-database setting in a future release. For now, the overlay uses the same default as stock Coolify.
...
>
> This is a Coolify core lifecycle issue. [corelix-platform](https://github.com/corelixio/platform) overlays `StartDatabaseProxy` for expanded port mapping and multi-port support, but the restart trigger path is upstream of that overlay. The proxy container should be restarted whenever the parent container restarts — either via Docker's `depends_on` or by hooking the restart event.
...
>
> [corelix-platform](https://github.com/corelixio/platform) fixes the classification via expanded `DATABASE_DOCKER_IMAGES` and the `coolify.database` label convention. Once the container is correctly classified as `ServiceDatabase`, status tracking uses the right container name.
...
>
> [corelix-platform](https://github.com/corelixio/platform) doesn't modify container naming. The addon's network management feature does use container names for DNS resolution within managed networks, so descriptive names would be beneficial.
...
- **Status:** Open
- **Relevance:** Corelix Platform implements a full network management system — server-level NetworkManager UI, per-resource NetworkAssignment UI, and environment-level auto-isolation.

**Draft Comment:**

> Implemented in [corelix-platform](https://github.com/corelixio/platform) as the Network Management feature.
>
> What it provides:
...
>
> [corelix-platform](https://github.com/corelixio/platform) handles this automatically via `resolveNetworkDriver()` — all network creation methods check whether the server is a Swarm manager (`docker info --format '{{.Swarm.ControlAvailable}}'`). Swarm managers get `--driver overlay`, standalone servers get `--driver bridge`. No manual driver selection needed.
>
...
>
> [corelix-platform](https://github.com/corelixio/platform) auto-detects the correct driver in `resolveNetworkDriver()`. On Swarm managers, all managed networks use `overlay` with `--attachable`. On standalone Docker, they use `bridge`. The driver selection is transparent — users don't need to specify it.
...
>
> [corelix-platform](https://github.com/corelixio/platform) provides the network layer:
...
>
> [corelix-platform](https://github.com/corelixio/platform) prevents this with environment network isolation. Each environment gets its own Docker bridge network (`ce-env-{uuid}`). DNS resolution is scoped to the network — containers in Environment A cannot resolve container names in Environment B. In `strict` mode, the addon also disconnects containers from the default `coolify` network, eliminating the shared namespace entirely.
...
>
> [corelix-platform](https://github.com/corelixio/platform) overlays `StartDatabaseProxy` for port mapping and multi-port proxy, but inherits the same image tag. A pinned image reference (e.g., `nginx:stable-alpine@sha256:...`) would prevent the overwrite.
...
>
> [corelix-platform](https://github.com/corelixio/platform) uses the same team resolution chain for resource backup downloads. The addon's `ResourceBackupController` traverses `resource → environment → project → team` explicitly. If you're hitting this on stock Coolify, it's likely a missing relationship in the download endpoint's authorization check.
