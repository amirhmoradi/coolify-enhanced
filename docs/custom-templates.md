...
This guide covers how to create, host, and use custom service templates with Corelix Platform. Custom template sources let you add external GitHub repositories containing docker-compose templates that appear alongside Coolify's built-in one-click services in the **New Resource** page.
...
You **don't** need to specify `# type:` if your database image is already in Corelix Platform's expanded recognition list (50+ images including Memgraph, Milvus, Qdrant, Neo4j, Cassandra, etc.). The classification is automatic.
...
By default, all enabled template sources are automatically synced every 6 hours. This is configurable via the `CORELIX_TEMPLATE_SYNC_FREQUENCY` environment variable (cron expression):

```bash
# Every 6 hours (default)
CORELIX_TEMPLATE_SYNC_FREQUENCY="0 */6 * * *"

# Every hour
CORELIX_TEMPLATE_SYNC_FREQUENCY="0 * * * *"

# Disable auto-sync
CORELIX_TEMPLATE_SYNC_FREQUENCY=
```
