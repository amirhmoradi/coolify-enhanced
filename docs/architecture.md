...
This document describes the architecture of the Corelix Platform package.
...
Application Boot
      │
      ▼
CorelixPlatformServiceProvider::register()
      │
      ├── Merge configuration
      │
      ▼
CorelixPlatformServiceProvider::boot()
      │
      ├── Load migrations
...
```php
// In CorelixPlatformServiceProvider::registerPolicies()
...
install.sh
      ...
      ├── Set CORELIX_PLATFORM=true in .env
...
The MCP (Model Context Protocol) server is a standalone TypeScript/Node.js application that wraps Coolify's REST API and corelix-platform's API endpoints, enabling AI assistants to manage infrastructure through natural language.
...
│                                                                 │
│  ┌──────────────────┐    ┌──────────────────────────┐         │
│  │  Coolify v4 API   │    │  corelix-platform API     │         │
...
index.ts::main()
      ...
      │
      ├── Enhanced detection:
      │     ├── CORELIX_PLATFORM=true in env? → enhanced = true
...
- **Timeout**: Default 30 seconds, configurable via `CORELIX_MCP_TIMEOUT`
...
| **npx-first distribution** | Zero-config installation: `npx @corelixio/platform-mcp` |
