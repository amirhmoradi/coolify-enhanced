...
This document describes the REST API endpoints provided by the Corelix Platform package, and the MCP (Model Context Protocol) server for AI-assisted infrastructure management.
...
The Corelix Platform MCP Server wraps all the above REST API endpoints (plus Coolify's native API) as MCP tools, enabling AI assistants to manage infrastructure through natural language.
...
| `CORELIX_PLATFORM` | No | `false` | Force enable enhanced tools (auto-detected if omitted) |
...
| **Permissions** | **5** | **corelix-platform API** |
| **Resource Backups** | **5** | **corelix-platform API** |
| **Custom Templates** | **7** | **corelix-platform API** |
| **Networks** | **10** | **corelix-platform API** |

72 core tools work with any Coolify v4 instance. 27 enhanced tools require the corelix-platform addon.
...
The MCP server automatically detects whether the corelix-platform addon is installed by probing `GET /api/v1/resource-backups`:
