...
- **Theme registry** — `config/corelix-platform.php` defines `ui_theme.themes` with slug, label, description, CSS path, and optional font label.
...
2. **Register in config** — Add entry to `config/corelix-platform.php` under `ui_theme.themes`:
   ```php
   '{slug}' => [
       'label' => 'Theme Name',
       'description' => '...',
