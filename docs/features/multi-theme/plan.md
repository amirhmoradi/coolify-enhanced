...
- `getAvailableThemes()` — Returns `config('corelix-platform.ui_theme.themes')`
...
$ceThemeConfig = $ceActiveTheme ? config("corelix-platform.ui_theme.themes.{$ceActiveTheme}") : null;
@if($ceThemeConfig)
...
```dockerfile
RUN mkdir -p /var/www/html/public/vendor/corelix-platform/themes
COPY resources/assets/themes/ /var/www/html/public/vendor/corelix-platform/themes/
```
...
```bash
php artisan migrate --path=vendor/corelix-io/platform/database/migrations
