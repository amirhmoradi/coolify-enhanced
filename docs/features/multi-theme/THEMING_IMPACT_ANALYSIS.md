...
This section assesses what Coolify exposes for white-labeling and what the corelix-platform
...
What corelix-platform can do today without any new overlays:
...
This way:
- The `base.blade.php` overlay reads `config('corelix-platform.whitelabel.brand_name')`
  instead of hardcoding 'Coolify'
- Logo/favicon paths are configurable without file replacement
- Documentation links use `config('corelix-platform.whitelabel.docs_url')` as base
...
Most already exist in corelix-platform for other features.
