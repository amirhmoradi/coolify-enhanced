# MCP Server for Corelix Platform — Product Requirements Document

## Problem Statement

Managing Coolify infrastructure through the web UI is manual and time-consuming. AI assistants (Claude Desktop, Cursor, VS Code Copilot, Kiro IDE) can automate infrastructure management through natural language, but need a structured interface to interact with Coolify's API. While the open-source `dazeb/coolify-mcp-enhanced` project provides a basic MCP server for vanilla Coolify, it does not support the enhanced features provided by the corelix-platform package (granular permissions, encrypted backups, resource backups, custom templates, network management, enhanced database classification).
...
2. **Expose corelix-platform features** as additional MCP tools — permissions management, resource backups, custom template sources, network management, and S3 encryption configuration
...
2. Calls **Coolify's REST API** (both native v1 and corelix-platform v1 endpoints) using Bearer token authentication
3. Lives in a `mcp-server/` directory within the corelix-platform monorepo
4. Is published as `@corelixio/platform-mcp` npm package

```
AI Client (Claude Desktop, Cursor, etc.)
    ↕ stdio (JSON-RPC)
MCP Server (@corelixio/platform-mcp)
    ↕ HTTPS (REST API)
Coolify Instance (with corelix-platform addon)
```
...
#### Category 2: Enhanced Permissions Tools (corelix-platform API)
...
#### Category 3: Resource Backup Tools (corelix-platform API)
...
#### Category 4: Custom Template Tools (corelix-platform API)
...
#### Category 5: Network Management Tools (corelix-platform API)
...
**Decision:** One HTTP client class handles both native Coolify API and corelix-platform API endpoints.
...
**Decision:** Enhanced tools (permissions, backups, templates, networks) are registered only when `CORELIX_PLATFORM=true` is set, or auto-detected by probing `GET /api/v1/resource-backups`.

**Rationale:** The MCP server should work with both vanilla Coolify and corelix-platform. Users running standard Coolify should not see tools that won't work. Auto-detection probes the enhanced API endpoint on startup — HTTP 200/401/403 means the endpoint exists (enhanced available), while 404 means standard Coolify.
...
**Core tools: 72** (work with any Coolify v4). **Enhanced tools: 27** (require corelix-platform addon).
...
```bash
# Global install
npm install -g @corelixio/platform-mcp
...
| `CORELIX_PLATFORM` | No | auto-detect | Force enable enhanced features (`true`) or auto-detect |
| `CORELIX_MCP_TIMEOUT` | No | `30000` | API request timeout in milliseconds |
| `CORELIX_MCP_RETRIES` | No | `3` | Retry attempts for transient failures |
...
- [ ] Enhanced permission tools work with corelix-platform addon
- [ ] Resource backup tools work with corelix-platform addon
- [ ] Custom template tools work with corelix-platform addon
- [ ] Network management tools work with corelix-platform addon
