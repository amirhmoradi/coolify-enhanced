...
Build a TypeScript MCP server that wraps Coolify's native REST API and corelix-platform's additional API endpoints, enabling AI assistants to manage infrastructure through natural language.
...
export function createMcpServer(client: CoolifyClient, enhanced: boolean): McpServer {
  ...
  // Enhanced tools (only when corelix-platform is detected)
  if (enhanced) {
...
`COOLIFY_MCP_LOG_LEVEL` was dropped. `CORELIX_MCP_TIMEOUT` and `CORELIX_MCP_RETRIES` were added for request configuration.
