...
EOF
    ...
    if [ -f "$ENV_FILE" ]; then
        if grep -q "CORELIX_PLATFORM" "$ENV_FILE"; then
            sed -i 's/CORELIX_PLATFORM=.*/CORELIX_PLATFORM=true/' "$ENV_FILE"
            info "Updated CORELIX_PLATFORM=true in .env"
        else
            echo "CORELIX_PLATFORM=true" >> "$ENV_FILE"
            info "Added CORELIX_PLATFORM=true to .env"
        fi
    else
    ...
    if docker exec coolify php artisan package:discover 2>/dev/null | grep -q "corelix-platform"; then
