...
export function registerEnvVarTools(server: McpServer, client: CoolifyClient): void {
  ...
  server.tool(
    ...
    {
      app_uuid: z.string().uuid().describe("Application UUID"),
      env_uuid: z.string().uuid().describe("Environment variable UUID"),
    },
  ...
  server.tool(
    ...
    { uuid: z.string().uuid().describe("Service UUID") },
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    async ({ uuid }) => {
      try {
  ...
  server.tool(
    ...
    {
      service_uuid: z.string().uuid().describe("Service UUID"),
      env_uuid: z.string().uuid().describe("Environment variable UUID"),
    },
    { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false },
