...
export function registerSecurityTools(server: McpServer, client: CoolifyClient): void {
  server.tool(
    ...
    async () => {
      try {
        ...
        const sanitized = keys.map(({ private_key, ...rest }: Record<string, unknown>) => rest);
  ...
  server.tool(
    ...
    { uuid: z.string().uuid().describe("Private key UUID") },
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    async ({ uuid }) => {
      try {
        const key = await client.getPrivateKey(uuid);
        const sanitized = { ...key, private_key: key.private_key ? "[REDACTED - use Coolify UI to view]" : undefined };
        return { content: [{ type: "text", text: JSON.stringify(sanitized, null, 2) }] };
      } catch (error) {
        return { content: [{ type: "text", text: `Error: ${(error as Error).message}` }], isError: true };
      }
