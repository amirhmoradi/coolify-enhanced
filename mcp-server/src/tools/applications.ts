...
export function registerApplicationTools(server: McpServer, client: CoolifyClient): void {
  ...
  server.tool(
    ...
    "Create a new application from a public git repository",
    {
      ...
      project_uuid: z.string().uuid().describe("Project UUID to deploy into"),
      server_uuid: z.string().uuid().describe("Server UUID to deploy on"),
  ...
  server.tool(
    ...
    {
      uuid: z.string().uuid().optional().describe("Resource UUID to deploy"),
