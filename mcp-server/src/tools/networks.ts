...
export function registerNetworkTools(server: McpServer, client: CoolifyClient): void {
  server.tool(
    ...
    { server_uuid: z.string().uuid().describe("Server UUID") },
  ...
  server.tool(
    ...
    {
      server_uuid: z.string().uuid().describe("Server UUID"),
      network_uuid: z.string().uuid().describe("Network UUID"),
    },
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  ...
  server.tool(
    ...
    "[Enhanced] List all networks a resource is attached to",
    {
      ...
      resource_uuid: z.string().uuid().describe("Resource UUID"),
    },
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  ...
  server.tool(
    ...
    "[Enhanced] Attach a resource to a managed Docker network",
    {
      resource_type: resourceTypeSchema,
      resource_uuid: z.string().uuid().describe("Resource UUID"),
      network_uuid: z.string().uuid().describe("Network UUID to attach"),
    },
    { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  ...
  server.tool(
    ...
    "[Enhanced] Detach a resource from a managed Docker network",
    {
      resource_type: resourceTypeSchema,
      resource_uuid: z.string().uuid().describe("Resource UUID"),
      network_uuid: z.string().uuid().describe("Network UUID to detach"),
    },
    { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    async ({ resource_type, resource_uuid, network_uuid }) => {
      try {
