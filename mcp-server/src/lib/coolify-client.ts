...
// corelix-platform API endpoints.
// ============================================================
...
export class CoolifyClient {
  ...
  constructor(config: CoolifyClientConfig) {
    ...
    if (this.baseUrl.startsWith("http://") && !process.env.CORELIX_ALLOW_INSECURE) {
      console.error(
        "WARNING: COOLIFY_BASE_URL uses HTTP. API tokens will be sent in plaintext. " +
        "Set CORELIX_ALLOW_INSECURE=true to suppress this warning."
      );
    }
  }
  ...
  private async requestWithRetry<T>(
    ...
    const isIdempotent = ["GET", "HEAD", "PUT", "DELETE"].includes(upperMethod);
    ...
    for (let attempt = 0; attempt <= this.retries; attempt++) {
      try {
        ...
        // Only retry non-idempotent methods (POST) on 429 (rate limit), not 5xx
        const canRetry = isIdempotent
          ? (!statusCode || isRetryableStatus(statusCode))
          : (statusCode === 429);
        if (attempt < this.retries && canRetry) {
  ...
  async listRegistries(): Promise<DockerRegistry[]> {
    return this.requestWithRetry<DockerRegistry[]>("GET", "/registries");
  }

  async createRegistry(data: CreateRegistryInput): Promise<DockerRegistry> {
    return this.requestWithRetry<DockerRegistry>("POST", "/registries", { body: data });
  }

  async getRegistry(uuid: string): Promise<DockerRegistry> {
    return this.requestWithRetry<DockerRegistry>("GET", `/registries/${uuid}`);
  }

  async updateRegistry(uuid: string, data: UpdateRegistryInput): Promise<DockerRegistry> {
    return this.requestWithRetry<DockerRegistry>("PUT", `/registries/${uuid}`, { body: data });
  }

  async deleteRegistry(uuid: string): Promise<void> {
    return this.requestWithRetry<void>("DELETE", `/registries/${uuid}`);
  }

  async testRegistry(uuid: string): Promise<RegistryTestResult> {
    return this.requestWithRetry<RegistryTestResult>("POST", `/registries/${uuid}/test`);
  }

  async syncRegistry(uuid: string): Promise<unknown> {
    return this.requestWithRetry<unknown>("POST", `/registries/${uuid}/sync`);
  }

  async getRegistrySyncStatus(uuid: string): Promise<RegistrySyncStatus[]> {
    return this.requestWithRetry<RegistrySyncStatus[]>("GET", `/registries/${uuid}/status`);
  }

  async syncAllRegistries(): Promise<unknown> {
    return this.requestWithRetry<unknown>("POST", "/registries/sync-all");
  }
