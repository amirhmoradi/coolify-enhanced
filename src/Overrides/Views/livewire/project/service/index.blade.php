            ...
            @if ($resourceType === 'application')
                ...
                <form wire:submit='submitApplication'>
                    ...
                    @feature('TRAEFIK_LABEL_OVERRIDES')
                        {{-- Corelix Enhanced: Traefik label overrides section for service application --}}
                        <div class="mt-4">
                            <details class="group">
                                <summary class="flex items-center gap-2 cursor-pointer select-none py-2 text-sm font-medium text-neutral-400 hover:text-white list-none">
                                    ...
                                    <span>Traefik Label Overrides</span>
                                    @if ($serviceApplication->label_overrides)
                                        <span class="px-1.5 py-0.5 text-xs rounded bg-coolgray-300 text-warning font-normal">Active</span>
                                    @endif
                                </summary>
                                <div class="mt-3 flex flex-col gap-3 pl-2">
                                    <p class="text-xs text-neutral-400 max-w-lg">
                                        Override any auto-generated Traefik label for the <strong class="text-white font-mono">{{ $serviceApplication->name }}</strong> container.
                                        Your entries win over Coolify's generated ones. Use <code class="text-coolgray-100 bg-coolgray-400 px-1 rounded">key: value</code> YAML format.
                                        <br>
                                        <strong class="text-neutral-300">Note:</strong> Router names include the Service UUID (visible in this page's URL).
                                    </p>

                                    @if (!empty($generatedLabels))
                                        <details class="text-xs">
                                            <summary class="cursor-pointer text-neutral-500 hover:text-neutral-300 select-none">
                                                View currently generated labels ({{ count($generatedLabels) }})
                                            </summary>
                                            <div class="mt-1 p-2 bg-coolgray-400 rounded font-mono text-xs text-neutral-300 overflow-auto max-h-48 space-y-0.5">
                                                @foreach ($generatedLabels as $label)
                                                    <div class="break-all">{{ $label }}</div>
                                                @endforeach
                                            </div>
                                        </details>
                                    @endif
                                    ...
                                    <div class="max-w-2xl">
                                        <x-forms.textarea
                                            ...
                                            id="labelOverridesYaml"
                                            label="Label Overrides (YAML)"
                                            placeholder="traefik.http.routers.https-0-SERVICE_UUID-kong.rule: &quot;Host(\`example.com\`) && PathPrefix(\`/supabase\`)&quot;"
                                            rows="6"
                                        />
                                    </div>

                                    @if ($labelOverridesError)
                                        <p class="text-sm text-red-400">{{ $labelOverridesError }}</p>
                                    @endif
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <x-forms.button
                                            canGate="update"
                                            :canResource="$serviceApplication"
                                            wire:click="saveLabelOverridesApplication"
                                        ...
                                        @if ($serviceApplication->label_overrides)
                                            <x-modal-confirmation
                                                title="Clear Label Overrides?"
                                                buttonTitle="Clear Overrides"
                                                isErrorButton
                                                submitAction="clearLabelOverridesApplication"
                                                :actions="['All Traefik label overrides for this container will be removed. Auto-generated labels will be used exclusively.']"
                                                confirmationText="{{ $serviceApplication->name }}"
                                                confirmationLabel="Enter the container name to confirm"
                                                shortConfirmationLabel="Container Name" />
                                        @endif
                                    </div>
                                </div>
                            </details>
                    ...
                    @else
                        {{-- Upsell placed OUTSIDE PREMIUM markers so it ships in free builds --}}
                    @endfeature
                </form>
                ...
                @else
                    <form wire:submit='submitDatabase'>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-col gap-2">
                                {{-- Corelix Enhanced: Multi-port proxy section --}}
                                ...
                                @else
                                    ...
                                    @endif
                                @endif
                                {{-- End Corelix Platform: Multi-port proxy section --}}
                            </div>
                        </div>
                        ...
                        @feature('TRAEFIK_LABEL_OVERRIDES')
                            {{-- Corelix Enhanced: Traefik label overrides section for service database --}}
                            <div class="mt-4">
                                <details class="group">
                                    <summary class="flex items-center gap-2 cursor-pointer select-none py-2 text-sm font-medium text-neutral-400 hover:text-white list-none">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform group-open:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <span>Traefik Label Overrides</span>
                                        @if ($serviceDatabase->label_overrides)
                                            <span class="px-1.5 py-0.5 text-xs rounded bg-coolgray-300 text-warning font-normal">Active</span>
                                        @endif
                                    </summary>
                                    <div class="mt-3 flex flex-col gap-3 pl-2">
                                        <p class="text-xs text-neutral-400 max-w-lg">
                                            Override any auto-generated Traefik label for the <strong class="text-white font-mono">{{ $serviceDatabase->name }}</strong> container.
                                            Your entries win over Coolify's generated ones. Use <code class="text-coolgray-100 bg-coolgray-400 px-1 rounded">key: value</code> YAML format.
                                        </p>
                                        ...
                                        <div class="max-w-2xl">
                                            <x-forms.textarea
                                                canGate="update"
                                                :canResource="$serviceDatabase"
                                                id="labelOverridesYaml"
                                                label="Label Overrides (YAML)"
                                                placeholder="traefik.http.routers.https-0-SERVICE_UUID-mydb.rule: &quot;Host(\`example.com\`) && PathPrefix(\`/db\`)&quot;"
                                                rows="6"
                                            />
                                        </div>
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            <x-forms.button
                                                canGate="update"
                                                :canResource="$serviceDatabase"
                                                wire:click="saveLabelOverridesDatabase"
                                                wire:loading.attr="disabled"
                                            >
                                                Save &amp; Apply
                                            </x-forms.button>
                                            @if ($serviceDatabase->label_overrides)
                                                <x-modal-confirmation
                                                    ...
                                                    buttonTitle="Clear Overrides"
                                                    isErrorButton
                                                    submitAction="clearLabelOverridesDatabase"
                                                    :actions="['All Traefik label overrides for this container will be removed. Auto-generated labels will be used exclusively.']"
                                                    confirmationText="{{ $serviceDatabase->name }}"
                                                    confirmationLabel="Enter the container name to confirm"
                                                    shortConfirmationLabel="Container Name" />
                                            @endif
                                        </div>
